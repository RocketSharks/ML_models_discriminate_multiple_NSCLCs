'''
Author: lx
Date: 2024-03-22 10:10:22
LastEditors: lx
LastEditTime: 2024-10-14 17:04:58
Description: 
FilePath: /ML_models_discriminate_multiple_NSCLCs/GBDT.py
'''
...
if __name__ == '__main__':
    ...
    gbdt = GradientBoostingClassifier(random_state=random_seed_)
    # 定义超参数超参数空间
    param_grid = {
        "n_estimators": np.arange(5, 10, step=1),         
        "learning_rate": [0.01, 0.1, 0.2, 0.3],              
        "max_depth": list(np.arange(1, 2, step=1)) + [None], 
        "subsample": [0.6],                       
    }

    cv = StratifiedKFold(n_splits=5)
    GS = GridSearchCV(gbdt, param_grid, cv=cv, n_jobs=-1, return_train_score=True,refit=True)  
    ...
    joblib.dump(GS.best_estimator_, f"GBDT.pkl")
    ...
    # 保存结果
    val_results_df.to_csv('./GBDT_val_results.csv', index=False)
    test_results_df.to_csv('./GBDT_test_results.csv', index=False)
    time_end = time.time()
